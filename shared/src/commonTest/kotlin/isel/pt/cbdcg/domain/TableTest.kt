...
class TableTest {
    ...
    fun `table is available when it has less than four participants`() {
        ...
        val participants = listOf(
            Participant(owner, Role.PLAYER, false),
            Participant(user(2u, "bea", "bea@email.com"), Role.PLAYER, false),
            Participant(user(3u, "cai", "cai@email.com"), Role.SPECTATOR, false),
        )
    ...
    fun `table is not available when it has four participants`() {
        ...
        val participants = listOf(
            Participant(owner, Role.PLAYER, false),
            Participant(user(2u, "bea", "bea@email.com"), Role.PLAYER, false),
            Participant(user(3u, "cai", "cai@email.com"), Role.PLAYER, false),
            Participant(user(4u, "dio", "dio@email.com"), Role.SPECTATOR, false),
        )
